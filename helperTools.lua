...
function imgui.OnDrawFrame()
    ...
    if main_window_state.v then
        imgui.LockPlayer = true
        imgui.SetNextWindowSize(imgui.ImVec2(780, 220), imgui.Cond.FirstUseEver)
        ...
        imgui.InputText(
            u8 "Кол-во миллисекунд, для задержки авто.ловли. " ..
            u8 "На данный момент " .. ini.main.time_wait .. "MC.", timeWait,
            imgui.InputTextFlags.CharsDecimal)
        imgui.SameLine()
        if imgui.Button(u8 "Сохранить значение задержки") then
            ini.main.time_wait = timeWait.v
            inicfg.save(ini, iniFileName)
        end
        imgui.SameLine()
        imgui.TextQuestion("( ? )", u8 "1.000 миллисекунд -> 1 секунда. Писать только цифры.")
        imgui.Separator()
        if imgui.Checkbox(u8 'Авто.ловля', hask_state) then
            ini.main.hask_state = hask_state.v
            inicfg.save(ini, iniFileName)
        end
        imgui.SameLine()
        ...
        imgui.SameLine()
        if imgui.Checkbox(u8 'Авто.лив после /hc off', auto_leave_from_game) then
