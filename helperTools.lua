...
local ini = inicfg.load({
    main = {
        ...
        auto_leave = false,
        line_break = true,
        hidd_new_hask_window = false,
        receive_updates = true,
        updated_question_opening_mode = false,
...
local update_url = 'https://raw.githubusercontent.com/Isk-On/HelperToolsAutoUpdate/main/updateIni.ini'
local update_path = getWorkingDirectory() .. "/updateIni.ini"
local script_url = 'https://raw.githubusercontent.com/Isk-On/HelperToolsAutoUpdate/main/helperTools.lua'
...
local hidd_new_hask_window = imgui.ImBool(ini.main.hidd_new_hask_window)
local updated_question_opening_mode = imgui.ImBool(ini.main.updated_question_opening_mode)
...
function main()
    ...
    while true do
        ...
        if wasKeyPressed(VK_F2) and mute_state then
            sampSendChat("/hmute " .. mute_id .. " " .. mute_quantity .. " " .. mute_reason)
            mute_state = false
        end
        ...
        if wasKeyPressed(VK_Q) and not sampIsCursorActive() and ini.main.key_open_hask then
            if not ini.main.updated_question_opening_mode then
                sampSendChat('/ha')
            end
            if ini.main.updated_question_opening_mode then
                if last_question_asked ~= 0 then
                    sampSendChat('/ha')
                    sampSendDialogResponse(772, 1, last_question_asked - 1, _)
                else
                    sampAddChatMessage(tag .. "Вопросы отсутствуют.", -1)
                end
            end
        end
...
function sampev.onShowDialog(id, style, title, b1, b2, text)
    ...
    if id == 774 and not ini.main.hidd_new_hask_window then
...
function sampev.onSendDialogResponse(id, button, list, input)
    if id == 773 and button == 1 and list == 0 and not ini.main.hidd_new_hask_window then
...
function imgui.OnDrawFrame()
    ...
    if main_window_state.v then
        imgui.LockPlayer = true
        imgui.SetNextWindowSize(imgui.ImVec2(780, 215), imgui.Cond.FirstUseEver)
        ...
        if imgui.Checkbox(u8 'Обновленный режим ловли', updated_question_opening_mode) then
            ini.main.updated_question_opening_mode = updated_question_opening_mode.v
            inicfg.save(ini, iniFileName)
        end
        imgui.SameLine()
        imgui.TextQuestion("( ? )",
            u8 "После нажатия на Q автоматически открывается последний вопрос. Идея хелпера: Liam_Capone. Beta")
        imgui.SameLine()
        if imgui.Checkbox(u8 'Скрыть измененное окно вопроса', hidd_new_hask_window) then
            ini.main.hidd_new_hask_window = hidd_new_hask_window.v
            inicfg.save(ini, iniFileName)
        end
        imgui.SameLine()
        imgui.TextQuestion("( ? )",
            u8 "Скрывает измененное окно с вопросом. Понадобится, если Вы хотите использовать тулс Ленса Стерлинга и этот одновременно, то включите этот пункт, но функции этого скрипта по типу авто.перенос, авто.приветствие не будут работать. Идея хелпера: Liam_Capone.")
            imgui.Separator()
...
function sampev.onServerMessage(color, text)
    ...
    if text:find('%[Вопрос%] Игрок (%w+_%w+)%[(%d+)%] задал вопрос введите %{f9b820%}/hask %(/ha%) %{33CCFF%}чтобы ответить. Всего вопросов: (%d+)') then
        ...
        last_question_asked = v
        return false
    end
    if text:find("Хелпер (%w+_%w+)%[(%d+)%] для") then
        last_question_asked = last_question_asked - 1
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateIni.versInfo.vers) > script_vers then
                ...
                sampAddChatMessage(tag .. "...{FFFFFF}введите /aboutUpdate чтобы увидеть список нововведений.",
                    -1)
                update_state = true
            else
                sampAddChatMessage(
